  ...
  static Future<void> run(String ragUrl) async {
    ...
    await for (final file in libDir.list(recursive: true)) {
      if (file is File && file.path.endsWith('.dart')) {
        ...
        for (final m in matches) if (m.group(1) != null) extractedStrings.add(m.group(1)!);
      }
    }
    ...
    print('✅ Extracted ${extractedStrings.length} strings. Translating via AI...');
...
class AccessibilityScanner {
  static Future<void> run() async {
    print('♿ A11y Scanner: Checking for European Accessibility Act compliance...');
    final libDir = Directory('lib');
    await for (final file in libDir.list(recursive: true)) {
      if (file is File && file.path.endsWith('.dart')) {
        final content = file.readAsStringSync();
...
class PerfAdvisor {
  static Future<void> run() async {
    ...
    print('💡 Suggestion: Use `const` constructors or `obx()` more granularly.');
  }
}
